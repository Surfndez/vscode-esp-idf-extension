...
describe("SDKConfig Editor", () => {
  ...
  it("Check int default value", async () => {
    const elem = await view.findWebElement(By.xpath(`.//input[@data-config-id='BOOTLOADER_WDT_TIME_MS']`));
    const result = await elem.getAttribute("value");
    expect(result).has.string("9000");
  });

  it("Check hex default value", async () => {
    const elem = await view.findWebElement(By.xpath(`.//input[@data-config-id='PARTITION_TABLE_OFFSET']`));
    const result = await elem.getAttribute("value");
    expect(result).has.string("0x8000");
  });

  it("Check bool default value", async () => {
    const elem = await view.findWebElement(By.xpath(`.//input[@data-config-id='BOOTLOADER_FACTORY_RESET']`));
    const result = await elem.getAttribute("value");
    expect(result).has.string("on");
  });

  it("Check choice has options and default value", async () => {
    const elem = await view.findWebElement(By.xpath(`.//select[@data-config-id='bootloader-config-vddsdio-ldo-voltage']`));
    const result = await elem.getAttribute("value");
